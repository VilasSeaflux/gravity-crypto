...
export default function AboutSecton() {
	...
	return (
		<section
			id="about"
			className="flex flex-col justify-center items-center gap-y-10 w-screen min-h-screen"
		>
			<h1
				id="about-heading"
				className="mt-10 font-light font-meta text-7xl text-center text-white"
			>
				About Gravity Team
			</h1>
			...
			<div className="place-content-center gap-1 grid grid-cols-4 bg-gradient-to-br from-violet-400 via-blue-500 to-orange-300 mx-auto mt-5 border-transparent w-[80%]">
				{data.map((item) => (
					<AboutBox
						key={`${item.title}_${item.number}`}
						title={item.title}
						number={item.number}
						about={item.about}
						dollar={item.dollar}
